...
function FigStack() {
  return (
    <svg viewBox="0 0 240 200" fill="none" className="h-44 w-full text-hairline-strong">
      <g stroke="currentColor" strokeWidth="1" strokeLinejoin="round">
        <path d="M120 34 210 76 120 118 30 76z" className="text-ink-tertiary" stroke="currentColor" />
        ...
        <path d="M30 76v62M210 76v62M120 118v62" strokeDasharray="2 4" opacity="0.5" />
      </g>
    </svg>
  );
}
...
function FigCubes() {
  const cube = (x: number, y: number, s: number, o = 1) => {
    ...
    return (
      <g opacity={o}>
        <path d={`M${x} ${y} ${x + s} ${y + h} ${x} ${y + s} ${x - s} ${y + h}z`} />
        <path d={`M${x - s} ${y + h} ${x - s} ${y + h + s} ${x} ${y + s + s} ${x} ${y + s}z`} />
        <path d={`M${x + s} ${y + h} ${x + s} ${y + h + s} ${x} ${y + s + s} ${x} ${y + s}z`} />
      </g>
    );
  };
        ...
        {Array.from({ length: 9 }).map((_, i) => {
          ...
          return (
            <g key={i} opacity={0.55 + i * 0.045}>
              <path d={`M${x} ${y} ${x + d} ${y} ${x + d} ${150} ${x} ${150}z`} />
              <path d={`M${x} ${y} ${x + d} ${y} ${x + d + 8} ${y - 5} ${x + 8} ${y - 5}z`} />
              <path d={`M${x + d} ${y} ${x + d + 8} ${y - 5} ${x + d + 8} ${145} ${x + d} ${150}z`} />
            </g>
          );
        })}
      </g>
...
const FIGURES = [
  {
    ...
    body: "Sable reads your resume, GitHub repos, and links to ground every question in what you've actually shipped.",
  },
  {
    tag: "FIG 0.2",
    Art: FigCubes,
      ...
      <div className="grid grid-cols-1 gap-px overflow-hidden md:grid-cols-3">
        {FIGURES.map(({ tag, Art, title, body }, i) => (
          ...
          >
            ...
            <p className="mt-2.5 text-[15px] leading-relaxed text-ink-subtle">{body}</p>
          </div>
        ))}
      </div>
    </section>
